# Extract useful features from candidate profiles
# These features are used later for ranking
import re
import datetime as dt
import jd_rubric as R

TODAY = dt.date(2026, 6, 27)
# Convert date string into Python date
...
# Join multiple strings and convert them to lowercase
def _lower_join(*parts):
    return " ".join(p for p in parts if p).lower()

# Find matching terms in the given text
def _term_hits(text, terms):
    return [t for t in terms if t in text]

# Combine all candidate text into one string
def extract_text_blob(cand):
    ...
    # Add skills to the combined text
...
def extract_career_narrative(cand):
    # Get only career details without skills
    p = cand.get("profile", {})
...
def skill_lookup(cand):
    # Store skills in a dictionary for quick search
    out = {}
    for s in cand.get("skills", []):
        out[s.get("name", "").strip().lower()] = s
    return out

# Calculate average time spent in each company
...
def detect_honeypot(cand):
    # Check whether the candidate profile looks suspicious
    reasons = []
    skills = cand.get("skills", [])
    yoe = cand.get("profile", {}).get("years_of_experience", 0) or 0

    # Check if expert skills have very little experience
    ...
    #  Any single skill's duration_months exceeds total years of experience
    for s in skills:
    ...
    # Career history total duration far exceeds stated years_of_experience
...
def extract_features(cand):
    # Extract all important features from one candidate
   ...
   # Detect keyword stuffing
    ...
    # Check machine learning experience
   ...
   # Calculate location score
    ...
    #  Honeypot ( Final suspicious profile check)
